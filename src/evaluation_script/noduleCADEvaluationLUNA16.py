...
def computeFROC_bootstrap(FROCGTList,FROCProbList,FPDivisorList,FROCImList,excludeList,numberOfBootstrapSamples=1000, confidence = 0.95):
    ...
    print ('computing FROC: bootstrap')
    for i in tqdm(range(numberOfBootstrapSamples)):
        #print ('computing FROC: bootstrap {}/{}'.format(i,numberOfBootstrapSamples))
...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
                ...
                if dist < radiusSquared:
                    if (noduleAnnot.state == "Included"):
                        ...
                        if key not in candidates2.keys():
                            print ("This is strange: CAD mark {} detected two nodules! Check for overlapping nodule annotations, SeriesUID: {}, nodule Annot ID: {}".format(str(candidate.id), seriesuid, str(noduleAnnot.id)))
                        else:
                            del candidates2[key]
    ...
    if int(totalNumberOfNodules) > 0:
        ...
        # The commercial CAD FROC
        from scipy.interpolate import interp1d
        commercial_fps = [0, 0.125, 0.25, .5, 1, 2, 3.1, 4, 5,6, 7, 8, 10, 20]
        isicad = [0, 0.09, 0.15, 0.24, 0.38, 0.53, 0.58, 0.64, 0.66, 0.725, 0.75, 0.75, .75,.75]
        herakles = [0, 0.2, 0.44, 0.58, 0.73, 0.785, 0.83, 0.84, 0.85, 0.85, 0.85, 0.85, .85, .85]

        # Use 1D spline to interpolate the commercial data
        f_isicad = interp1d(commercial_fps, isicad)
        f_herakles = interp1d(commercial_fps, herakles)

        # Top 10 result - https://luna16.grand-challenge.org/result/?id=DIAG_CONVNET_2C_FPRED_V2
        contest_fps = [0.125, 0.25, 0.5, 1., 2., 4., 8.]
        diag_convnet_fpred_v2 = [0.669, 0.760, 0.831, 0.892, 0.923, 0.944, 0.960]

        #plt.plot(fps_itp, sens_itp, color=clr, label="%s" % CADSystemName, lw=2)
        plt.plot(fps_itp, sens_itp, color=clr, label='DSE Capstone Model', lw=2)
        plt.plot(fps_itp, f_isicad(fps_itp), color='r', label='ISICAD')
        plt.plot(fps_itp, f_herakles(fps_itp), color='g', label='Herakles CAD')
        plt.plot(contest_fps, diag_convnet_fpred_v2, color='c', label='Top 10 CNN')
        if performBootstrapping:
        ...
        plt.savefig(os.path.join(outputDir, "froc_%s.png" % CADSystemName), bbox_inches=0, dpi=600)

        pts = []
        for i in [0.125, 0.25, 0.5, 1, 2, 4, 8]:  # False positive thresholds
            pt = np.mean(sens_bs_mean[[np.where((fps_bs_itp >= (i-0.001)) & (fps_bs_itp <= (i+0.001)))[0]]])
            pts.append(pt)

        print('CPM = {}, pts = {}'.format(np.mean(pts), pts))
...
if __name__ == '__main__':
    ...
    annotations_filename          = 'annotations/annotations.csv' #sys.argv[1]
    annotations_excluded_filename = 'annotations/annotations_excluded.csv' #sys.argv[2]
    seriesuids_filename           = 'annotations/seriesuids.csv' #sys.argv[3]
    results_filename              = 'combine_predictions/predictions_ALL.csv' #sys.argv[4]
    #results_filename              = 'exampleFiles/submission/predictions_ALL_9_23_combined.csv' #sys.argv[4]
    #results_filename              = 'exampleFiles/submission/sampleSubmission.csv' #sys.argv[4]
    outputDir                     = 'output' #sys.argv[5]
    # execute only if run as a script
    print("Using predictions from file: {}".format(results_filename))
    noduleCADEvaluation(annotations_filename,annotations_excluded_filename,seriesuids_filename,results_filename,outputDir)
    print ("Finished! I predicted on file {}.".format(results_filename))
    print("The results are in {}".format(outputDir))
