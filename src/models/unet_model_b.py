...
CHECKPOINT_FILENAME = "./cnn_3DUNET_ModelB_exp1_lr009_BSexp1_64_64_64_HOLDOUT{}".format(HOLDOUT_SUBSET) + time.strftime("_%Y%m%d_%H%M%S") + ".hdf5"
...
def get_random_idx(idx, batch_size = 20):
	...
	bs = (batch_size//4)
	idx0_shuffle = idx0[0:bs]
	idx1_shuffle = idx1[0:(batch_size-bs)]
...
def generate_data(hdf5_file, batch_size=50, subset=0, validation=False):
	...
	while True:
		...
		# Normalization to Hunsfield units
...
# Hunsfield normalization is ignored in this ModelB
def normalize(img):
...
with h5py.File(path_to_hdf5, 'r') as hdf5_file: # open in read-only mode
	...
	model.compile(optimizer=keras.optimizers.Adam(lr=0.009),
				  ...
				  # loss={'PredictionMask': dice_coef_loss, 'PredictionClass': 'binary_crossentropy'}, loss_weights={'PredictionMask': 1.0, 'PredictionClass': 0.2},
	...
	model.save('UNET_modelB_exp1_lr009_BSexp1_H{}.h5'.format(HOLDOUT_SUBSET))
	model.save_weights('UNET_weights_modelB_exp1_lr009_BSexp1_H{}.h5'.format(HOLDOUT_SUBSET))
